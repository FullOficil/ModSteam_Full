...
let migracaoLoginsPromise = null;
...
if (firebaseDb) {
  migracaoLoginsPromise = migrarLoginsRegistradosParaRaiz().catch((error) => {
    console.error(
      "Não foi possível mover LOGINS_REGISTRADOS para a raiz:",
      resumirErro(error)
    );
    return false;
  });
...
app.post("/api/cadastrar-conta", async (req, res) => {
  ...
  try {
    ...
    const pagamentosSnapshot = await firebaseDb
      .ref(`${FIREBASE_LOJA_PATH}/PagamentosProcessados`)
      .once("value");
    const pagamentosProcessados = pagamentosSnapshot.val();
...
async function migrarLoginsRegistradosParaRaiz() {
  if (!firebaseDb) return false;

  const caminhoAntigo = `${FIREBASE_LOJA_PATH}/${FIREBASE_LOGINS_PATH}`;
  const snapshotAntigo = await firebaseDb.ref(caminhoAntigo).once("value");
  const loginsAntigos = snapshotAntigo.val();

  if (!temDadosLoginsRegistrados(loginsAntigos)) {
    return false;
  }

  await firebaseDb.ref(FIREBASE_LOGINS_PATH).transaction((loginsAtuais) =>
    mesclarLoginsRegistrados(loginsAtuais, loginsAntigos),
  undefined, false);

  await firebaseDb.ref(caminhoAntigo).remove();
  console.log("LOGINS_REGISTRADOS foi movido de LojaDayZombi para a raiz do Firebase.");
  return true;
}

async function aguardarMigracaoLogins() {
  if (migracaoLoginsPromise) {
    await migracaoLoginsPromise;
  }
}

function mesclarLoginsRegistrados(loginsRaiz, loginsAntigos) {
  const raiz = clonarObjeto(loginsRaiz);
  const antigos = clonarObjeto(loginsAntigos);

  return {
    ...antigos,
    ...raiz,
    USUARIOS: {
      ...clonarObjeto(antigos.USUARIOS),
      ...clonarObjeto(raiz.USUARIOS)
    },
    NICKS: {
      ...clonarObjeto(antigos.NICKS),
      ...clonarObjeto(raiz.NICKS)
    },
    UID_PARA_NICK: {
      ...clonarObjeto(antigos.UID_PARA_NICK),
      ...clonarObjeto(raiz.UID_PARA_NICK)
    }
  };
}

function temDadosLoginsRegistrados(logins) {
  const dados = clonarObjeto(logins);
  return Object.keys(clonarObjeto(dados.USUARIOS)).length > 0 ||
    Object.keys(clonarObjeto(dados.NICKS)).length > 0 ||
    Object.keys(clonarObjeto(dados.UID_PARA_NICK)).length > 0;
}
...
async function obterContaRegistradaPorUid(uid) {
  ...
  await aguardarMigracaoLogins();
  const snapshot = await firebaseDb
    .ref(`${FIREBASE_LOGINS_PATH}/USUARIOS/${uid}`)
    .once("value");
  return snapshot.val();
}
...
async function registrarPagamentoNoFirebase(pagamento, produto) {
  ...
  const resultado = await lojaRef.transaction((estadoAtual) => {
    ...
    delete estado[FIREBASE_LOGINS_PATH];
  ...
  if (firebaseUid) {
    try {
      await registrarEventoPagamentoNaConta(pagamento, produto, contabilizar, agora);
    } catch (error) {
      console.error(
        "Pagamento registrado, mas o histórico da conta não foi atualizado:",
        resumirErro(error)
      );
    }
  }
...
async function registrarEventoPagamentoNaConta(
  pagamento,
  produto,
  contabilizar,
  agora = Date.now()
  ...
  const firebaseNome = extrairNomeFirebaseDoPagamento(pagamento);
  const contaRef = firebaseDb.ref(
    `${FIREBASE_LOGINS_PATH}/USUARIOS/${firebaseUid}`
  );

  const resultado = await contaRef.transaction((contaAtual) => {
    const contaRegistrada = clonarObjeto(contaAtual);
