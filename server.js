...
// O catálogo é carregado quando a página é aberta/recarregada.
// Não mantemos cache com TTL para que uma atualização manual da página
// consulte os dados atuais. Se várias requisições chegarem exatamente juntas,
// elas compartilham apenas a MESMA leitura em andamento.
let carregamentoLojaPublica = null;
...
const PRODUTOS = Object.freeze({
  ...
  "titulos-250": Object.freeze({
    id: "titulos-250",
    titulo: "250 Títulos",
    descricao: "Pacote com 250 Títulos para sua conta Day Zombi Survival.",
    categoria: "titulos",
    valor: 24.99,
    moeda: "BRL",
    titulos: 250,
  ...
  "titulos-775": Object.freeze({
    id: "titulos-775",
    titulo: "775 Títulos",
    descricao: "Pacote com 775 Títulos para sua conta Day Zombi Survival.",
    categoria: "titulos",
    valor: 74.99,
    moeda: "BRL",
    titulos: 775,
  ...
  "titulos-1200": Object.freeze({
    id: "titulos-1200",
    titulo: "1.000 + 200 BÔNUS",
    descricao: "Receba 1.000 Títulos + 200 Títulos de bônus. Total: 1.200 Títulos.",
    categoria: "titulos",
    valor: 99.99,
    moeda: "BRL",
    titulos: 1200,
  ...
  "titulos-2500": Object.freeze({
    id: "titulos-2500",
    titulo: "2.000 + 500 BÔNUS",
    descricao: "Receba 2.000 Títulos + 500 Títulos de bônus. Total: 2.500 Títulos.",
    categoria: "titulos",
    valor: 199.99,
    moeda: "BRL",
    titulos: 2500,
...
// Produtos antigos ficam fora da vitrine, mas continuam reconhecidos ao
// confirmar pagamentos iniciados antes desta atualização.
const PRODUTOS_LEGADOS = Object.freeze({
...
function obterProdutoFixoParaConfirmacao(produtoId) {
  return PRODUTOS[produtoId] || PRODUTOS_LEGADOS[produtoId] || null;
}
...
app.get("/api/configuracao-publica", async (_req, res) => {
  ...
  const lojaPublica = await obterLojaPublicaComCache();
  ...
  res.json({
    ...
    produtos: lojaPublica.produtos,
    vendaChave: lojaPublica.vendaChave
  });
});

// Rota mantida por compatibilidade. A página normal não faz polling periódico.
app.get("/api/produtos", async (_req, res) => {
  res.set("Cache-Control", "no-store");
  const lojaPublica = await obterLojaPublicaComCache();
  return res.json(lojaPublica);
});
...
app.get("/api/minha-conta", async (req, res) => {
  ...
  try {
    ...
    // A tela da conta só precisa de Dados. Não baixa Histórico/Eventos.
    const dados = await obterDadosContaPorUid(usuario.uid);
    const contaLeve = { Dados: dados || {} };

    return res.json({
      autenticado: true,
      cadastrado: Boolean(dados?.nick),
      usuario: usuarioPublico(usuario),
      conta: contaPublica(contaLeve)
    });
  } catch (error) {
...
app.post("/api/cadastrar-conta", async (req, res) => {
  ...
  try {
    ...
    // Lê somente Dados da conta. Não baixa HistoricoDeCompras/Eventos/etc.
    const dadosRef = firebaseDb.ref(
      `${FIREBASE_LOGINS_PATH}/USUARIOS/${uidConta}/Dados`
    );
    const dadosExistentesSnapshot = await dadosRef.once("value");
    const dadosExistentes = dadosExistentesSnapshot.val() || {};
    ...
    if (dadosExistentes?.nick) {
      return res.json({
        ...
        cadastrado: true,
        usuario: usuarioPublico(usuario),
        conta: contaPublica({ Dados: dadosExistentes })
      });
    }
    ...
    // Reserva SOMENTE o Nick escolhido. Não transaciona LOGINS_REGISTRADOS inteiro.
    const reservaNick = await nickRef.transaction((registroAtual) => {
      if (registroAtual?.uid && registroAtual.uid !== uidConta) {
        return;
      }

      if (registroAtual?.uid === uidConta) {
    ...
    // Transaciona somente /USUARIOS/<UID>/Dados.
    // Histórico, eventos e outros nós da conta não entram na leitura/transação.
    const resultadoDados = await dadosRef.transaction((dadosAtual) => {
      const dados = dadosAtual && typeof dadosAtual === "object"
        ? { ...dadosAtual }
        : {};

      // Outra requisição do mesmo usuário pode ter concluído antes.
      if (dados.nick) return dados;

      const saldoAtual =
        dados.Titulos !== undefined && dados.Titulos !== null
      ...
      return {
        ...
        Titulos: Number.isFinite(saldoAtual) ? Math.max(0, saldoAtual) : 0,
        criadoEm: Number(dados.criadoEm) || agora,
        ultimoLoginEm: agora,
        atualizadoEm: agora
      };
    }, undefined, false);

    if (!resultadoDados.committed) {
    ...
    const dadosFinal = resultadoDados.snapshot.val() || {};
    const nickFinalKey = String(
      dadosFinal.nickChave || String(dadosFinal.nick || "").toLowerCase()
    ).trim().toLowerCase();

    // Duas requisições simultâneas do mesmo UID podem tentar Nicks diferentes.
    // Só o Nick efetivamente salvo na conta permanece reservado.
    if (nickFinalKey !== nickInfo.nickKey) {
      ...
      await garantirIndicesDaContaPorDados(uidConta, dadosFinal).catch((error) => {
        console.warn(
          "[LOJA] Não foi possível reparar os índices da conta:",
          resumirErro(error)
        );
      });
    } else {
      // Finaliza os dois índices com um update multipath pequeno e atômico.
      await firebaseDb.ref().update({
        ...
        [`${FIREBASE_LOGINS_PATH}/UID_PARA_NICK/${uidConta}`]: {
          nickChave: nickInfo.nickKey,
          nick: nickInfo.nick
        }
      });
    ...
    return res.status(201).json({
      ...
      cadastrado: true,
      usuario: usuarioPublico(usuario),
      conta: contaPublica({ Dados: dadosFinal })
    });
  } catch (error) {
    return responderErroAutenticacao(res, error);
  }
...
app.get("/api/minha-carteira", async (req, res) => {
  ...
  try {
    const usuario = await obterUsuarioAutenticado(req, true);
    const conta = await obterContaLojaPorUid(usuario.uid);
    if (!conta?.Dados?.nick) {
...
app.post("/api/processar-pagamento", async (req, res) => {
  ...
  try {
    ...
    // Compra normal/chave precisa somente de Dados. Promoção também precisa
    // dos marcadores/histórico para impedir compra duplicada.
    conta = produto.promocao
      ? await obterContaLojaPorUid(usuario.uid)
      : { Dados: (await obterDadosContaPorUid(usuario.uid)) || {} };
  } catch (error) {
    return responderErroAutenticacao(res, error);
  }
  ...
  try {
    ...
    if (statusPagamento === "approved") {
      // Só toca na conta do jogador quando o Mercado Pago realmente aprovou.
      ...
      // Para pagamentos ainda não aprovados, salva somente um espelho técnico
      // pequeno. Não lê nem transaciona a conta do jogador.
      await salvarEspelhoPagamentoLeve(
        pagamento,
        produto,
        usuario.uid,
        conta?.Dados?.nick || ""
        ...
        console.warn(
          "[LOJA] Não foi possível salvar o espelho leve do pagamento:",
          resumirErro(error)
        );
      });
    }
...
app.get("/api/verificar-pagamento", async (req, res) => {
  ...
  try {
    ...
    let produto = obterProdutoDoPagamento(pagamento);
    let espelho = null;
    let uidEspelho = "";

    // Caminho normal: pagamentos criados por esta loja já carregam UID e produto
    // na metadata do Mercado Pago. Nesse caso, enquanto estiver pending/in_process,
    // NÃO fazemos nenhuma leitura no Realtime Database.
    if (uidPagamento !== usuario.uid || !produto) {
      espelho = await obterEspelhoPagamento(paymentId);
      uidEspelho = limparTexto(espelho?.uid, 160);

      if (!produto) {
        produto = obterProdutoDoEspelhoPagamento(espelho);
      }
    }
    ...
    if (!pagamentoPertenceAoUsuario) {
      return res.status(403).json({
        aprovado: false,
        erro: "Esse pagamento não pertence à sua conta."
      });
    }
    ...
    // Só lê/transaciona a conta depois da aprovação real.
    if (aprovado && produto) {
      registro = await registrarPagamentoNoFirebase(
        pagamento,
        produto,
        usuario.uid
      );
    ...
    console.log("[LOJA] Verificação de pagamento:", {
      ...
      consultouEspelhoFirebase: Boolean(espelho),
      registrado: registro?.registrado ?? false,
      deltaTitulos: registro?.deltaTitulos ?? 0
    });
...
async function garantirIndicesDaContaPorDados(uid, dados) {
  const nick = String(dados?.nick || "").trim();
  const nickChave = String(
    dados?.nickChave || nick.toLowerCase()
  ).trim().toLowerCase();
  ...
  if (!reserva.committed) {
    console.error(
      `[LOJA] Conflito de índice: o Nick ${nickChave} pertence a outro UID.`
    );
    return;
  }
...
// Leitura mínima para telas/ações que só precisam do perfil e saldo.
async function obterDadosContaPorUid(uid) {
  if (!firebaseDb || !uid) return null;
  const snapshot = await firebaseDb
    .ref(`${FIREBASE_LOGINS_PATH}/USUARIOS/${uid}/Dados`)
    .once("value");
  return snapshot.val();
}

// A carteira precisa de três ramos específicos. Lemos somente eles em paralelo
// em vez de baixar qualquer outro conteúdo que possa existir sob o UID.
async function obterContaLojaPorUid(uid) {
  if (!firebaseDb || !uid) return null;

  const base = `${FIREBASE_LOGINS_PATH}/USUARIOS/${uid}`;
  const [dadosSnapshot, historicoSnapshot, promocoesSnapshot] = await Promise.all([
    firebaseDb.ref(`${base}/Dados`).once("value"),
    firebaseDb.ref(`${base}/HistoricoDeCompras`).once("value"),
    firebaseDb.ref(`${base}/Eventos/PromocoesAdquiridas`).once("value")
  ]);

  return {
    Dados: dadosSnapshot.val() || {},
    HistoricoDeCompras: historicoSnapshot.val() || {},
    Eventos: {
      PromocoesAdquiridas: promocoesSnapshot.val() || {}
    }
  };
}
...
async function salvarEspelhoPagamentoLeve(pagamento, produto, uid, nick = "") {
  if (!firebaseDb || !pagamento || !produto || !uid) return;
  ...
  await firebaseDb
    ...
    .update({
      ...
      titulos: Math.max(0, Math.trunc(Number(produto.titulos) || 0)),
      status,
...
function obterProdutoDoEspelhoPagamento(espelho) {
  ...
  const produtoFixo = obterProdutoFixoParaConfirmacao(produtoId);
...
function aplicarCompraTitulosNaConta(contaAtual, contexto) {
  ...
  // IMPORTANTE: transações do Realtime Database podem chamar o callback
  // inicialmente com null mesmo quando o dado já existe no servidor.
  // Portanto, null NÃO pode abortar a compra. O chamador fornece uma
  // cópia conhecida da conta para a primeira tentativa; quando o servidor
  // devolver o estado real, o Firebase executará novamente este callback.
  const contaBase = contaAtual && typeof contaAtual === "object"
    ? contaAtual
    : clonarObjeto(contexto?.contaConhecida);

  if (!contaBase || typeof contaBase !== "object" ||
      !contaBase.Dados || typeof contaBase.Dados !== "object" ||
      !contaBase.Dados.nick) {
    return;
  }

  const novaConta = { ...contaBase };
  const dados = clonarObjeto(contaBase.Dados);
  const historico = clonarObjeto(contaBase.HistoricoDeCompras);
  const compraExistente = clonarObjeto(historico[pagamentoId]);

  const eventos = clonarObjeto(contaBase.Eventos);
  ...
  if (aprovadoMercadoPago && produto.vendaChave) {
    ...
    if (registroAcesso?.acessoVinculado) {
      ...
      const resultado = await contaRef.transaction((contaAtual) => {
        ...
        if (!contaBase.Dados || typeof contaBase.Dados !== "object" ||
            !contaBase.Dados.nick) {
          return;
        }
        ...
        const historico = clonarObjeto(contaBase.HistoricoDeCompras);
        const compraExistente = clonarObjeto(historico[pagamentoId]);
        if (compraExistente.creditoAplicado === true) return contaBase;
        ...
        return { ...contaBase, HistoricoDeCompras: historico };
      }, undefined, false);
...
function obterProdutoDoPagamento(pagamento) {
  ...
  const produtoFixo = obterProdutoFixoParaConfirmacao(id);
  if (produtoFixo) return produtoFixo;
...
async function obterLojaPublicaComCache() {
  // Sem cache persistente: cada nova abertura/reload recebe o catálogo atual.
  // A única deduplicação acontece enquanto uma leitura já está em andamento,
  // evitando leituras idênticas quando vários usuários abrem no mesmo instante.
  if (carregamentoLojaPublica) {
    return carregamentoLojaPublica;
  }

  carregamentoLojaPublica = (async () => {
    const [produtos, vendaChave] = await Promise.all([
      obterCatalogoPublico(),
      obterVendaChavePublica()
    ]);

    return { produtos, vendaChave };
  })();

  try {
    return await carregamentoLojaPublica;
  } finally {
    carregamentoLojaPublica = null;
  }
}
