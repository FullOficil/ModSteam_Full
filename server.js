...
app.get("/api/minha-carteira", async (req, res) => {
  ...
  try {
    ...
    const pagamentos = clonarObjeto(conta?.Eventos?.PagamentosLoja);
...
app.post("/api/processar-pagamento", async (req, res) => {
  ...
  try {
    ...
    // Registra também pagamentos pendentes para que o histórico exista desde
    // a criação. Quando virar "approved", a mesma transação aplica os Títulos.
    ...
    return res.status(201).json(resumirPagamento(pagamento, registro));
  } catch (error) {
...
app.get("/api/verificar-pagamento", async (req, res) => {
  ...
  try {
    ...
    return res.json(resumirPagamento(pagamento, registro));
  } catch (error) {
...
async function registrarPagamentoNoFirebase(pagamento, produto, uidForcado = "") {
  ...
  /*
   * Quando esta função é chamada logo depois do checkout, o produto e o UID
   * vieram da própria sessão autenticada do site. Nesse caso não dependemos
   * de campos opcionais que o Mercado Pago pode omitir/normalizar na resposta
   * imediata. O valor continua sendo conferido no servidor.
   *
   * No webhook, onde não existe sessão autenticada, mantemos a validação
   * completa pelo snapshot salvo no pagamento.
   */
  ...
  const agora = Date.now();
  const baseConta = `${FIREBASE_LOGINS_PATH}/USUARIOS/${uidConta}`;
  const titulosProduto = produto.categoria === "titulos"
    ? Math.max(0, Math.trunc(Number(produto.titulos) || 0))
    : 0;

  // ------------------------------------------------------------
  // PROMOÇÃO: uma compra por conta.
  // O mesmo paymentId pode repetir (webhook/polling) sem ser bloqueado.
  // ------------------------------------------------------------
  let promocaoJaAdquirida = false;
  if (produto.promocao && aprovadoMercadoPago) {
    const promoRef = firebaseDb.ref(
      `${baseConta}/Eventos/PromocoesAdquiridas/${produto.id}`
    );

    const promoResultado = await promoRef.transaction((registroAtual) => {
      const existente = clonarObjeto(registroAtual);
      if (existente.adquirida === true &&
          existente.pagamentoId &&
          String(existente.pagamentoId) !== pagamentoId) {
        return existente;
      }
      ...
      return {
        ...
        adquiridaEm: existente.adquiridaEm || obterTimestampAprovado(pagamento) || agora,
        atualizadoEm: agora
      };
    }, undefined, false);

    const promoFinal = promoResultado.snapshot.val();
    promocaoJaAdquirida = Boolean(
      promoFinal?.adquirida &&
      promoFinal?.pagamentoId &&
      String(promoFinal.pagamentoId) !== pagamentoId
    );
  }

  const beneficioDeveFicarAtivo = aprovadoMercadoPago && !promocaoJaAdquirida;
  let deltaTitulos = 0;
  let saldoTitulos = obterSaldoTitulosConta(conta);
  let titulosAplicados = 0;

  // ------------------------------------------------------------
  // TÍTULOS: transação SOMENTE em /Dados.
  // É aqui que o jogo lê o saldo:
  // LOGINS_REGISTRADOS/USUARIOS/<UID>/Dados/Titulos
  //
  // CreditosLoja funciona como trava idempotente. Se o webhook ou o polling
  // repetir o mesmo pagamento, ele NÃO soma os Títulos duas vezes.
  // ------------------------------------------------------------
  if (produto.categoria === "titulos" && titulosProduto > 0) {
    const dadosRef = firebaseDb.ref(`${baseConta}/Dados`);
    const processamentoId = crypto.randomUUID();
    const titulosDesejados = beneficioDeveFicarAtivo ? titulosProduto : 0;

    const dadosResultado = await dadosRef.transaction((dadosAtual) => {
      if (!dadosAtual || typeof dadosAtual !== "object" || !dadosAtual.nick) {
        return;
      }

      const dados = { ...dadosAtual };
      const creditosLoja = clonarObjeto(dados.CreditosLoja);
      const creditoExistente = clonarObjeto(creditosLoja[pagamentoId]);
      const aplicadoAntes = Math.max(0, Number(creditoExistente.titulosAplicados) || 0);

      let saldoAtual;
      if (dados.Titulos !== undefined && dados.Titulos !== null) {
        saldoAtual = Number(dados.Titulos);
      } else {
        saldoAtual = Number(dados.Dolls);
      }
      if (!Number.isFinite(saldoAtual)) saldoAtual = 0;
      saldoAtual = Math.max(0, saldoAtual);

      const delta = titulosDesejados - aplicadoAntes;
      ...
      const novoCredito = {
        ...
        produtoTitulo: produto.titulo,
        titulos: titulosProduto,
        titulosAplicados: titulosDesejados,
        status,
        atualizadoEm: agora,
        criadoEm: creditoExistente.criadoEm || agora
      };

      // Identifica exatamente qual chamada alterou o saldo. Assim conseguimos
      // informar delta=0 em reprocessamentos sem risco de crédito duplicado.
      if (delta !== 0) {
        novoCredito.ultimoProcessamentoId = processamentoId;
        novoCredito.ultimoDelta = delta;
        novoCredito.ultimoProcessamentoEm = agora;
      }

      creditosLoja[pagamentoId] = novoCredito;

      return {
        ...dados,
        Titulos: novoSaldo,
        CreditosLoja: creditosLoja,
        atualizadoEm: agora
      };
    }, undefined, false);

    if (!dadosResultado.committed) {
      console.error("[LOJA] Não foi possível atualizar Dados/Titulos.", {
        pagamentoId,
        uidConta,
        status
      });
      return {
        registrado: false,
        motivo: "O pagamento foi localizado, mas o saldo de Títulos não pôde ser atualizado.",
        deltaTitulos: 0,
        saldoTitulos
      };
    }

    saldoTitulos = Number(dadosResultado.snapshot.child("Titulos").val()) || 0;
    const creditoFinal = dadosResultado.snapshot
      .child(`CreditosLoja/${pagamentoId}`)
      .val() || {};
    titulosAplicados = Math.max(0, Number(creditoFinal.titulosAplicados) || 0);

    if (String(creditoFinal.ultimoProcessamentoId || "") === processamentoId) {
      deltaTitulos = Number(creditoFinal.ultimoDelta) || 0;
    }
  }

  // ------------------------------------------------------------
  // CHAVE: mantém o sistema já existente da loja antiga.
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // HISTÓRICO DA CONTA.
  // Este nó é criado automaticamente na primeira compra:
  // LOGINS_REGISTRADOS/USUARIOS/<UID>/Eventos/PagamentosLoja/<PAYMENT_ID>
  // ------------------------------------------------------------
  const historico = {
    pagamentoId,
    ...
    contabilizar: beneficioDeveFicarAtivo,
    creditoAplicado: produto.categoria === "titulos"
      ? Boolean(beneficioDeveFicarAtivo && titulosAplicados === titulosProduto)
      : Boolean(registroAcesso?.acessoVinculado),
    bloqueadoPromocaoJaAdquirida: promocaoJaAdquirida,
    aprovadoEm: obterTimestampAprovado(pagamento),
    criadoEm: agora,
    atualizadoEm: agora
  };

  try {
    const historicoRef = firebaseDb.ref(
      `${baseConta}/Eventos/PagamentosLoja/${pagamentoId}`
    );
    const historicoAnterior = (await historicoRef.once("value")).val() || {};
    historico.criadoEm = Number(historicoAnterior.criadoEm) || agora;
    historico.aprovadoEm = historico.aprovadoEm || historicoAnterior.aprovadoEm || null;
    await historicoRef.set({ ...historicoAnterior, ...historico });
  } catch (error) {
    console.error("[LOJA] Saldo processado, mas falhou ao salvar histórico:", {
      pagamentoId,
      uidConta,
    ...
    return {
      ...
      contabilizar: beneficioDeveFicarAtivo,
      promocaoJaAdquirida,
      motivo: "O saldo foi processado, mas o histórico ainda precisa ser sincronizado.",
      deltaTitulos,
      saldoTitulos,
  ...
  // Espelho global apenas para auditoria. Se ele falhar, não desfaz a compra
  // da conta do jogador, pois saldo + histórico já foram salvos no lugar certo.
  try {
  ...
  console.log("[LOJA] Compra sincronizada com a conta:", {
    pagamentoId,
    uidConta,
    produtoId: produto.id,
    status,
...
function obterIdsPromocoesAdquiridas(conta) {
  ...
  // Compatibilidade com compras promocionais feitas antes desta versão,
  // quando ainda não existia o marcador PromocoesAdquiridas.
  const pagamentos = clonarObjeto(conta?.Eventos?.PagamentosLoja);
...
function resumirPagamento(pagamento, registro = null) {
  const produto = obterProdutoDoPagamento(pagamento);
  const validacao = validarPagamento(pagamento, produto);
