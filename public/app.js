...
function cacheElements() {
  [
    ...
    "refreshWalletButton", "recoveryPaymentId", "recoverPaymentButton", "recoveryPaymentStatus",
    "logoutButton", "logoutNickButton", "checkoutModal", "checkoutTitle", "checkoutDescription",
...
function bindEvents() {
  ...
  els.recoverPaymentButton?.addEventListener("click", recoverPreviousPayment);
...
async function loadAccountState() {
  try {
    ...
    await refreshWallet();
    await syncStoredPendingPayment();
...
async function syncStoredPendingPayment() {
  if (!state.user) return;

  const paymentId = String(localStorage.getItem("dayzombiPendingPaymentId") || "").trim();
  if (!/^\d{3,30}$/.test(paymentId)) return;

  try {
    const data = await apiFetch(`/api/verificar-pagamento?payment_id=${encodeURIComponent(paymentId)}`);
    if (data.aprovado) {
      localStorage.removeItem("dayzombiPendingPaymentId");
      await refreshWallet();
      return;
    }
    ...
    console.warn("Não foi possível sincronizar o último pagamento automaticamente:", error);
  }
}

async function recoverPreviousPayment() {
  if (!els.recoveryPaymentId || !els.recoverPaymentButton || !els.recoveryPaymentStatus) {
    showToast("A sincronização manual não está disponível nesta versão da página.");
    return;
  }

  const paymentId = String(els.recoveryPaymentId.value || "").trim();
  if (!/^\d{5,30}$/.test(paymentId)) {
    els.recoveryPaymentStatus.textContent = "Informe um ID de pagamento válido.";
    return;
  }

  els.recoveryPaymentStatus.textContent = "Sincronizando...";
  els.recoverPaymentButton.disabled = true;

  try {
    const data = await apiFetch(`/api/verificar-pagamento?payment_id=${encodeURIComponent(paymentId)}`);

    if (data.aprovado) {
      els.recoveryPaymentStatus.textContent = data.vendaChave
        ? "Compra sincronizada. Sua chave foi atualizada."
        : `Compra sincronizada. ${formatNumber(data.deltaTitulos || data.titulos || 0)} Títulos confirmados.`;
      els.recoveryPaymentId.value = "";
      await refreshWallet();
      return;
    }

    if (String(data.status).toLowerCase() === "approved" || data.pagamentoAprovado) {
      els.recoveryPaymentStatus.textContent = data.mensagem || "Pagamento aprovado, mas ainda não foi possível aplicar o benefício.";
      await refreshWallet();
      return;
    }

    els.recoveryPaymentStatus.textContent = data.mensagem || `Status atual: ${statusLabel(data.status)}.`;
  } catch (error) {
    els.recoveryPaymentStatus.textContent = error.message;
  } finally {
    els.recoverPaymentButton.disabled = false;
  }
}
...
async function handlePaymentResult(data) {
  ...
  if (state.pendingPaymentId) {
    localStorage.setItem("dayzombiPendingPaymentId", String(state.pendingPaymentId));
  }
  ...
  // O Mercado Pago pode aprovar a cobrança antes de o Firebase conseguir
  // aplicar o benefício. Nesse caso NÃO oferecemos uma nova compra, para
  // evitar cobrança duplicada; tentamos sincronizar o mesmo pagamento.
  if (String(data.status).toLowerCase() === "approved" || data.pagamentoAprovado) {
    renderCreditPending(data);
    await refreshWallet();
    startPaymentPolling();
    return;
  }
...
function renderCreditPending(data) {
  els.paymentResult.innerHTML = `
    <h3>Pagamento aprovado — sincronizando</h3>
    <p>${escapeHtml(data.mensagem || "O pagamento foi aprovado, mas a conta ainda não foi atualizada.")}</p>
    <p><strong>Não faça outra compra.</strong> Vamos tentar sincronizar este mesmo pagamento.</p>
    <button class="button button--primary" type="button" data-sync-payment>Tentar sincronizar</button>
    <button class="button button--ghost" type="button" data-finish-purchase>Fechar</button>`;

  els.paymentResult.querySelector("[data-sync-payment]")?.addEventListener("click", async () => {
    const button = els.paymentResult.querySelector("[data-sync-payment]");
    ...
    try {
      const synced = await apiFetch(`/api/verificar-pagamento?payment_id=${encodeURIComponent(state.pendingPaymentId)}`);
      if (synced.aprovado) {
      ...
      renderCreditPending(synced);
      await refreshWallet();
    } catch (error) {
      showToast(error.message);
      if (button) {
        button.disabled = false;
        button.textContent = "Tentar sincronizar";
      }
    }
  });
  ...
  state.paymentPoll = setInterval(async () => {
    ...
    try {
      ...
      if (data.aprovado) {
        ...
        renderApproved(data);
        await refreshWallet();
      } else if (String(data.status).toLowerCase() === "approved" || data.pagamentoAprovado) {
        // Continua tentando automaticamente: pagamento aprovado não pode ficar
        // dependendo do jogador apertar um botão para receber os Títulos.
        renderCreditPending(data);
        await refreshWallet();
