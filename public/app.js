...
function cacheElements() {
  [
    ...
    "keySaleBanner", "keySaleBannerTitle", "keySaleBannerDescription", "keySaleBannerPrice", "keySaleBannerFlag", "keySaleBannerTimer", "keySaleViewButton",
...
function renderHistory() {
  ...
  els.purchaseHistory.innerHTML = history.map((item) => {
    ...
    const amountLabel = item.vendaChave || item.categoria === "chave"
      ? "Chave"
      : `${item.contabilizar ? "+" : ""}${formatNumber(item.titulos)} Títulos`;
    return `
      <article class="history-item">
        ...
        <div class="history-item__meta">
          <small>${date ? new Date(date).toLocaleString("pt-BR") : "—"}</small>
          <small>${formatMoney(item.valor, item.moeda)}</small>
        </div>
...
function renderKeySale() {
  ...
  els.keySaleBannerPrice.textContent = formatMoney(sale.valor, sale.moeda);
  els.keySaleBannerFlag.textContent = owned
    ? "Sua conta já possui acesso"
    : (sale.promocional ? "Oferta limitada" : "Acesso disponível");
...
function startPromotionCountdowns() {
  // Este intervalo atualiza SOMENTE os relógios/estado visual no navegador.
  // Não consulta servidor nem Firebase. Novas promoções só são carregadas
  // quando a página é aberta/recarregada.
  window.setInterval(() => {
    ...
    if (sale?.expiraEm) {
      ...
      if (Number(sale.expiraEm) <= Date.now() && !state.wallet?.acessoComprado) {
        els.keySaleBuyButton.disabled = true;
        els.keySaleBuyButton.textContent = "OFERTA ENCERRADA";
        els.keySaleViewButton.disabled = true;
      }
    }
  }, 1000);
}
...
function renderCreditPending(data) {
  els.paymentResult.innerHTML = `
    ...
    <p>${escapeHtml(data.mensagem || "Pagamento confirmado. Atualizando sua conta...")}</p>
    <p><strong>Atualizando Títulos e histórico automaticamente...</strong></p>`;
}
