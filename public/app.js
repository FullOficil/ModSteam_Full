...
const STORAGE_KEY = "dayzombi-pagamento-pendente-v5";
...
const accountGoogleIcon = document.querySelector("#account-google-icon");
...
const nicknamePanel = document.querySelector("#nickname-panel");
const nicknameForm = document.querySelector("#nickname-form");
const nicknameInput = document.querySelector("#nickname-input");
const nicknameError = document.querySelector("#nickname-error");
const nicknameAccountEmail = document.querySelector("#nickname-account-email");
...
let estadoConta = { cadastrado: false, conta: null };
let autenticacaoPronta = false;
let promessaCadastroNick = null;
let resolverCadastroNick = null;
...
nicknameForm?.addEventListener("submit", salvarCadastroNick);
nicknameCancel?.addEventListener("click", cancelarCadastroNick);
nicknameInput?.addEventListener("input", limparErroCadastroNick);
...
window.addEventListener("keydown", (event) => {
  ...
  if (nicknameModal && !nicknameModal.hidden) return;
...
async function garantirLogin(produtoId = "") {
  if (usuarioAtual) {
    const cadastrado = await garantirContaRegistrada();
    return cadastrado ? usuarioAtual : null;
  }
  ...
  try {
    ...
    if (error.code === "auth/popup-blocked") {
      throw new Error(
        "O navegador bloqueou a janela de login. Libere pop-ups para este site e tente novamente."
      );
    }
    ...
    if (error.code === "auth/web-storage-unsupported") {
      throw new Error(
        "Este navegador bloqueou o armazenamento necessário para manter o login."
      );
    }

    throw new Error(error.message || "Não foi possível entrar com o Google agora.");
  }
}
...
async function atualizarEstadoConta() {
  ...
  if (!usuarioAtual) {
    ...
    estadoConta = { cadastrado: false, conta: null };
    accountButton.classList.remove("account-button--owned", "account-button--connected");
    mostrarIconeGoogle();
    accountStatusText.textContent = "Login Google";
    accountLogout.hidden = true;
    fecharCadastroNick();
    concluirEsperaCadastroNick(false);
    atualizarBotoesCompra(false);
    liberarNickCheckout(false);
    return;
  }

  atualizarAvatarConta(usuarioAtual);
  accountStatusText.textContent = "Verificando conta...";
  accountLogout.hidden = false;
  accountButton.classList.add("account-button--connected");

  try {
    const token = await usuarioAtual.getIdToken();
    const contaResponse = await fetch("/api/minha-conta", {
      cache: "no-store",
      headers: { Authorization: `Bearer ${token}` }
    });
    const contaData = await contaResponse.json();

    if (contaResponse.status === 401) {
      await usuarioAtual.getIdToken(true);
      throw new Error(contaData.erro || "Sessão expirada.");
    }

    if (!contaResponse.ok) {
      throw new Error(contaData.erro || "Não foi possível consultar sua conta.");
    }

    estadoConta = contaData;
    if (!contaData.cadastrado) {
      estadoAcesso = { comprado: false };
      accountButton.classList.remove("account-button--owned");
      accountStatusText.textContent = "Escolha seu Nick";
      atualizarBotoesCompra(false);
      liberarNickCheckout(false);
      abrirCadastroNick();
      return;
    ...
    fecharCadastroNick();
    concluirEsperaCadastroNick(true);
    const nick = contaData.conta?.nick || "Jogador";
    localStorage.setItem(SUPPORTER_NAME_KEY, nick);
    liberarNickCheckout(true, nick);

    const acessoResponse = await fetch("/api/meu-acesso", {
      cache: "no-store",
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    if (!acessoResponse.ok) {
      throw new Error(acessoData.erro || "Acesso indisponível.");
    }

    estadoAcesso = acessoData;
    if (acessoData.comprado) {
      accountButton.classList.add("account-button--owned");
      accountStatusText.textContent = `${nick} — Acesso liberado`;
      preencherModalAcesso(acessoData);
      atualizarBotoesCompra(true);
    } else {
      accountButton.classList.remove("account-button--owned");
      accountStatusText.textContent = `${nick} — Conectado`;
      atualizarBotoesCompra(false);
    }
  } catch (error) {
    ...
    estadoAcesso = { comprado: false };
    accountButton.classList.remove("account-button--owned");
    accountStatusText.textContent = estadoConta.cadastrado
      ? `${estadoConta.conta?.nick || "Conta"} — Conectado`
      : "Conta conectada";
    atualizarBotoesCompra(false);
  }
}
...
async function acaoBotaoConta() {
  ...
  if (!estadoConta.cadastrado) {
    await garantirContaRegistrada();
    return;
  }
...
async function abrirCheckout(produtoId) {
  try {
    ...
    if (!usuarioAtual || !estadoConta.cadastrado) {
...
function atualizarCabecalhoCheckout(produto, valorAlternativo = 0) {
  ...
  if (usuarioAtual && estadoConta.cadastrado) {
    ...
    const nick = estadoConta.conta?.nick || "Jogador";
    const email = usuarioAtual.email || usuarioAtual.displayName || "Conta Google";
    checkoutAccountEmail.textContent = `${nick} • ${email}`;
  } else {
...
function processarPagamento(selectedPaymentMethod, formData) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!usuarioAtual || !estadoConta.cadastrado) {
        const error = new Error("Entre com o Google e escolha seu Nick antes de pagar.");
...
async function garantirContaRegistrada() {
  if (!usuarioAtual) return false;
  if (estadoConta.cadastrado) return true;

  await atualizarEstadoConta();
  if (estadoConta.cadastrado) return true;
  return aguardarCadastroNick();
}

function aguardarCadastroNick() {
  abrirCadastroNick();
  if (!promessaCadastroNick) {
    promessaCadastroNick = new Promise((resolve) => {
      resolverCadastroNick = resolve;
    });
  }
  return promessaCadastroNick;
}

function concluirEsperaCadastroNick(resultado) {
  if (resolverCadastroNick) resolverCadastroNick(Boolean(resultado));
  resolverCadastroNick = null;
  promessaCadastroNick = null;
}

function abrirCadastroNick() {
  if (!nicknameModal || !usuarioAtual) return;
  nicknameAccountEmail.textContent = usuarioAtual.email || "Conta Google";
...
function fecharCadastroNick() {
  if (!nicknameModal) return;
  nicknameModal.hidden = true;
  if (modal?.hidden !== false && accessModal?.hidden !== false) {
    document.body.classList.remove("checkout-open");
  }
}

async function salvarCadastroNick(event) {
  event.preventDefault();
  if (!usuarioAtual || !nicknameInput || !nicknameSave) return;

  const nick = normalizarNickContaCliente(nicknameInput.value);
  if (!nick) {
    mostrarErroCadastroNick(
      "Use de 3 a 20 caracteres: letras, números, _ ou -. Comece com letra ou número."
    );
    return;
  }
  ...
  nicknameSave.textContent = "Criando...";
  limparErroCadastroNick();

  try {
    const response = await fetch("/api/cadastrar-conta", {
      method: "POST",
      cache: "no-store",
      headers: {
        "Content-Type": "application/json",
    ...
    if (!response.ok || !data.cadastrado) {
      throw new Error(data.erro || "Não foi possível criar a conta.");
    }

    estadoConta = data;
    localStorage.setItem(SUPPORTER_NAME_KEY, data.conta?.nick || nick);
    nicknameInput.value = data.conta?.nick || nick;
    fecharCadastroNick();
    concluirEsperaCadastroNick(true);
    await atualizarEstadoConta();
  } catch (error) {
    mostrarErroCadastroNick(error.message || "Não foi possível criar a conta.");
  } finally {
    nicknameSave.disabled = false;
    nicknameCancel.disabled = false;
    nicknameSave.textContent = "Criar conta";
  }
}

async function cancelarCadastroNick() {
  fecharCadastroNick();
  concluirEsperaCadastroNick(false);
  sessionStorage.removeItem(LOGIN_INTENT_KEY);
  if (firebaseAuthClient) await firebaseAuthClient.signOut();
}

function normalizarNickContaCliente(valor) {
  const nick = String(valor || "").normalize("NFKC").trim();
  return /^[A-Za-z0-9][A-Za-z0-9_-]{2,19}$/.test(nick) ? nick : "";
}

function mostrarErroCadastroNick(texto) {
  nicknameInput.setAttribute("aria-invalid", "true");
  nicknameError.textContent = texto;
  nicknameError.classList.remove("oculto");
  nicknameInput.focus();
}

function limparErroCadastroNick() {
  nicknameInput?.removeAttribute("aria-invalid");
  if (nicknameError) {
    nicknameError.textContent = "";
    nicknameError.classList.add("oculto");
  }
}

function liberarNickCheckout(liberado, nick = "") {
  if (!supporterNameInput) return;
  supporterNameInput.readOnly = Boolean(liberado);
  if (liberado && nick) supporterNameInput.value = nick;
}

function mostrarIconeGoogle() {
  if (accountGoogleIcon) accountGoogleIcon.hidden = false;
...
function atualizarAvatarConta(usuario) {
  const foto = String(usuario?.photoURL || "").trim();
  if (foto && accountAvatarImage) {
    accountAvatarImage.src = foto;
    accountAvatarImage.hidden = false;
    if (accountGoogleIcon) accountGoogleIcon.hidden = true;
    return;
  }
  mostrarIconeGoogle();
}
...
function prepararNomeApoiador() {
  ...
  const nickConta = estadoConta.conta?.nick || "";
  if (nickConta) {
    supporterNameInput.value = nickConta;
    supporterNameInput.readOnly = true;
    return;
  }

  supporterNameInput.readOnly = false;
  supporterNameInput.value = localStorage.getItem(SUPPORTER_NAME_KEY) || "";
}
...
function validarNomeApoiador() {
  ...
  const nome = normalizarNomeApoiador(supporterNameInput.value);
  if (nome.length < 2) {
    mostrarErroNomeApoiador("Escolha seu Nick na conta antes de pagar.");
    return "";
  }
